...
macro_rules! component_pack_impl {
    ( $n:expr, $( $name:ident )+ ) => {
        /// Implementation for a tuple of components.
        impl<$($name: Component + 'static, )*> ComponentPack for ($($name,)*) {
            #[inline]
            fn is_valid(&self) -> bool {
                true
            }
        ...
        /// Implementation for a tuple of optional components.
        impl<$($name: Component + 'static, )*> ComponentPack for ($(Option<$name>,)*) {
            #[inline]
            fn is_valid(&self) -> bool {
                true
            }
            ...
            ) -> (ArchetypeId, usize)
            {
                ...
                let (archetype, index) = if let Some(archetype) = archetypes.get_archetype(&type_key) {
                    ...
                    // Initialize map values
                    $(
                        if archetype.type_key.contains(TypeId::of::<$name>()) {
                            let storage = if let Some(storage) = archetypes
                                .get_storage_mut::<$name>()
                            {
                                storage
